...
def main() -> None:
    ...
    # Gate the stop on completeness alone. The `met` "everything verified in recent activity"
    # check proved too noisy as a hard veto: it reads low for text/advice turns that have
    # nothing to verify, so it falsely held finished work again and again. Completeness already
    # encodes verification (its top rung is "done AND verified"), so genuinely unfinished work
    # scores below done_threshold and is still held; `met` now only colours the reason line.
