...
> **Route is on by default** (with a key) and gates **every** tool call — the `hooks.json`
> matcher is `*` and `route.tools` is empty. Because it can *block* a call, narrow
> `route.tools` (e.g. `["Bash"]`) or set `route.enabled: false` in `.jeveloper.json` if a Jev
> call on every read-only tool isn't worth the latency.
